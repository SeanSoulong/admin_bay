...
export default function AdminProductsTable({
  ...
  useEffect(() => {
    if (products.length > 0) {
      fetchUserDataForProducts();
    }
  }, [products]);

  const fetchUserDataForProducts = async () => {
    setLoadingUsers(true);
    try {
      const db = getDatabase();
      const uniqueUserIds = [
        ...new Set(products.map((p) => p.userId).filter(Boolean)),
      ];
      ...
      const userPromises = uniqueUserIds.map(async (userId) => {
        ...
        if (snapshot.exists()) {
          return { userId, data: snapshot.val() };
        }
      ...
      users.forEach((user) => {
        if (user && user.data) {
          userDataMap[user.userId] = user.data;
        }
      });
                    ...
                    >
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center">
                          ...
                          <div className="ml-3">
                            {/* HERE - truncate leading-tight is already on this div */}
                            <div className="text-sm w-20 font-medium text-gray-900 truncate leading-tight">
                              {loadingUsers ? (
                                <div className="animate-pulse h-4 w-24 bg-gray-200 rounded"></div>
                              ) : productUser?.first_name &&
                                productUser?.last_name ? (
